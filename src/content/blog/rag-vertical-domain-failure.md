...
<strong>⚠️ 注意</strong>
表格中的数据不是危言耸听。根据多篇 2024-2025 年的行业报告，通用 RAG 在垂直领域的准确率确实集中在 50%-70%。如果你的项目处于这个区间，不要 panic——这是行业常态，不是你的实现有问题。解决方案是降低预期，从"替代专家"调整为"辅助工具"。
</div>
...
<div class="callout callout-warning">
<strong>⚠️ 注意</strong>
"伪相关"问题是垂直领域 RAG 最容易踩的坑之一。一个常见但错误的想法是"换个更好的 Embedding 模型就能解决"——实际上，即使 100% 精度的向量检索也解决不了"语义相似但专业不相关"的问题。解决方案是引入多路召回（向量 + 关键词 + 知识图谱）和重排序。
</div>
...
<strong>💡 技巧</strong>
如果预算有限，可以建立"渐进式评估体系"：第一步用自动化指标（Hit Rate、MRR）跑通基线；第二步邀请 1-2 位领域专家对 Top-50 问题进行盲测；第三步在生产环境加"人工审核"环节。先用自动化发现明显问题，再用专家评估发现深层问题，最后用人工兜底保证安全。
</div>
